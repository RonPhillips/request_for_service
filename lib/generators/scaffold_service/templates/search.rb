...
# - is an alias for not.
# If no operator is present, the operator "and" is assumed.
...
#  ruby or chunky bacon => records where ruby appears or
#                          both chunky and bacon appear
...
#  (ruby or rails) -"ruby on rails" => records where ruby or rails appears
#                                      but not the phrase "ruby on rails" 
#
# Query feature by Nate McNamara (nate@mcnamara.net)
# Original TextSearch library by Duane Johnson.
# Modified to accept :select argument by Ron Phillips
# Aaron Bedras was an originator of this code as well
require 'set'
module ActiveRecord
  class Base
    def self.search_in_for(fieldsarray, termphrase, *args)
      self.searches_on(fieldsarray)
      return self.search(termphrase, *args)
    end
    
    # Allow the user to set the default searchable fields
    # Added code to cleanse args down to legal fields
    def self.searches_on(args)
      if not args.empty? 
        valids = valid_string_columns(args.collect { |f| f.to_s })
      end
      valids.nil? ?  @searchable_fields = self.string_column_names : @searchable_fields = valids
    ...
    def self.searchable_fields()
      ...
      @searchable_fields ||= self.string_column_names
      return @searchable_fields
    end
    
    #Allow the user to limit returned fields (minimum is 
    #searches_on set)
    def self.shows_on(fieldsarray)
      if not fieldsarray.empty?
        @showfields = valid_columns(fieldsarray.collect{ |f| f.to_s}).to_set
        @showfields = [].to_set if @showfields.nil?
        @showfields = self.searchable_fields.to_set | @showfields
        @showfields = @showfields.to_a
      else
        @showfields = self.column_names
       end
      @showfields
    end
    ...
    def self.search(text = nil, options = {})
      fields = options[:only] || searchable_fields()
      if options[:except]
      ...
      condition_list = []
      unless text.nil?
        text_condition = build_text_condition(fields, text)
        if text_condition && text_condition != '' &&
          text_condition != '()'
    ...
    def self.string_columns
      @string_columns ||= self.columns.select { |c|
        c.type == :text or c.type == :string
      }
      return @string_columns
    end
    def self.string_column_names
      return self.string_columns.collect{|column| column.name}
    end
    
    def self.valid_string_columns(fieldarray)
      return  self.string_column_names & fieldarray
    end
    
    def self.valid_columns(fieldarray)
      return  self.column_names & fieldarray
    end
